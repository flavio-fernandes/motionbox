...
Adafruit_MQTT_Subscribe service_sub_stealth = Adafruit_MQTT_Subscribe(&mqtt, DEV_PREFIX MQTT_SUB_STEALTH);
...
void myMqttLoop() {
    ...
    while ((subscription = mqtt.readSubscription())) {
        ...
        if (subscription == &service_sub_admin_flags) {
            ...
            sendOperState();
	    sendNoMotionState();
        } else if (subscription == &service_sub_stealth) {
            parseOnOffToggle(MQTT_SUB_STEALTH, message, setStealthOn, clearStealthOn, toggleStealthState);
	    updateMqttLed(mqtt.connected());
        ...
        } else if (subscription == &service_sub_ping) {
	    sendOperState();
	    sendNoMotionState();
    ...
    if (lastConnected != currConnected) {
        ...
        if (currConnected) {
            ...
            if (!mqtt.subscribe(&service_sub_admin_flags) ||
                !mqtt.subscribe(&service_sub_stealth) ||
...
static void updateMqttLed(bool currMqttConnected) {
    const bool mqttLedOn = currMqttConnected && !getStealthState();
    digitalWrite(PIN_MQTT_LED, mqttLedOn ? PIN_MQTT_LED_ON : PIN_MQTT_LED_OFF);
}
...
static bool checkMqttConnected() {
    ...
    // do not attempt to reconnect if there are reconnect ticks outstanding
    ...
    updateMqttLed(currMqttConnected);
...
void mqtt1SecTick() {
    ...
    if (! mqtt.connected()) {
        ...
        Serial.print(" stealth_state: "); Serial.print(getStealthState() ? "on" : "off");
        Serial.println("");
    }
#endif
...
void sendNoMotionState() {
    ...
    if (!service_pub_no_motion_minutes.publish(noMotionMins)) {
#ifdef DEBUG
        Serial.println("Unable to publish no motion mins state");
#endif
        mqtt.disconnect();
    }
}
